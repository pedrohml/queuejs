...
export class API {
    static produce(app: INestApplication, topic: string, payload: MessageCollection): request.Test {
    ...
    static register(app: INestApplication, group: string, topic: string): request.Test {
        return request(app.getHttpServer())
            .post(`/api/groups/${group}/topics/${topic}/register`)
            .expect(201);
    }

    static commit(app: INestApplication, group: string, topic: string, offset: number): request.Test {
        return request(app.getHttpServer())
    ...
    static consume(app: INestApplication, group: string, topic: string, count: number): request.Test {
        return request(app.getHttpServer())
            .get(`/api/groups/${group}/topics/${topic}/messages/${count}`)
            .expect(200);
    }

    static consumeNext(app: INestApplication, group: string, topic: string): request.Test {
        return request(app.getHttpServer())
