...
describe('producer + consumer (e2e)', () => {
    ...
    it('produce 1, consume 1', async () => {
        ...
        await API.consumeNext(app, group, topic)
            .expect({ messages: [{ data: data1, topic, offset: 1 }] });
    });
    ...
    it('produce 1, consume 1, commit 1, consume none', async () => {
        ...
        await API.commit(app, group, topic, 1)
    ...
    it('produce 1, consume 1, missed commit, consume 1, commit 1, consume 2', async () => {
        ...
        await API.produce(app, topic, { messages: [ { data: data1 } ]} as MessageCollection);
        await API.produce(app, topic, { messages: [ { data: data2 } ]} as MessageCollection);
        ...
        await API.consumeNext(app, group, topic)
            .expect({ messages: [{ data: data2, topic, offset: 2 }] });
    });
    ...
    it('produce 1, consume [1, 2]', async () => {
        ...
        await API.consume(app, group, topic, 10)
            .expect({ messages: [{ data: data1, topic, offset: 1 }, { data: data2, topic, offset: 2 }] });
    });
    ...
    it('produce 1, consume [1, 2], commit 2, consume [3, 4]', async () => {
        ...
        await API.consume(app, group, topic, 1)
            .expect({ messages: [
        ...
        await API.consume(app, group, topic, 3)
            .expect({ messages: [
    ...
    it('produce 1, consume [1, 2], commit 3 (skip), consume 4', async () => {
        ...
        await API.consume(app, group, topic, 2)
            .expect({ messages: [
                { data: data1, topic, offset: 1 },
                { data: data2, topic, offset: 2 }
            ] });
        ...
        await API.consume(app, group, topic, 10)
            .expect({ messages: [ { data: data4, topic, offset: 4 } ] });
    });
